...
Technician needed ASAP"""
    ...
    for job in jobs:
        st.write(f"**{job['id']}** - {job['property']} • Unit {job['unit']}")
        st.write(f"*{job['issue']}* - {job['priority']} - {job['status']}")
        st.divider()
...
<div style="text-align: center; color: #64748b; padding: 20px;">
    ...
    <p style="font-size: 0.9rem;">🤖 Powered by DeepSeek AI • © 2024 {COMPANY_NAME}</p>
</div>
""", unsafe_allow_html=True)
